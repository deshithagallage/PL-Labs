...
class ASTParsser:
    ...
    def read(self):
        ...
        if self.current_token.type in [lexical2.TokenType.ID, lexical2.TokenType.INT,
                                       lexical2.TokenType.STRING] :
    ...
    def procE(self):
        ...
        match self.current_token.value:
            ...
            case 'fn':
                ...
                while self.current_token.type == lexical2.TokenType.ID or self.current_token.value == '(':
                    self.procVb()
                    n += 1
    ...
    def procTc(self):
        ...
        self.procB()
        print('Tc->B')
        if self.current_token.type == lexical2.TokenType.TERNARY_OPERATOR:
            self.read()
            self.procTc()
    ...
    def procR(self):
        ...
        while (self.current_token.type in [lexical2.TokenType.ID, lexical2.TokenType.INT,
                                           lexical2.TokenType.STRING] or self.current_token.value in ['true', 'false',
                                                                                                        'nil', 'dummy',
                                                                                                        "("]):
            self.procRn()
            print('R->R Rn')
    ...
    def procDb(self):
        ...
        elif self.current_token.type == lexical2.TokenType.ID:
            self.read()

            if self.current_token.type == lexical2.TokenType.COMMA:
                # Db -> Vl '=' E => '='
                self.read()
                self.procVb()
    ...
    def procVb(self):
        print('procVb')
        if self.current_token.type == lexical2.TokenType.ID:
            self.read()
            print('Vb->id')
            # self.buildTree("id", 1)
...
tokens = []
# tokenize input
tokenizer = lexical2.Tokenizer(program)
token = tokenizer.get_next_token()
print(token.type, token.value)
while token.type != lexical2.TokenType.EOF:
    print(token.type, token.value)
    if token.value in lexical2.RESERVED_KEYWORDS:
        token.type = lexical2.TokenType.RESERVED_KEYWORD
